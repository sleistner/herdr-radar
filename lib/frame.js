...
class Frame {
  ...
  async render(now) {
    ...
    const headers = grouped && config.groupHeaders;
    const heads = headers ? state.groupBoundaries(displayEntries).heads : new Set();
    ...
    for (const entry of entries) {
      ...
      // Without headers only the tree itself indents: a member one level in.
      const nestDepth = keys.nested(entry.workspace) ? 1 : 0;
      const depth = headers ? (heads.has(entry.pane) ? 0 : 1) + nestDepth : grouped ? nestDepth : 0;
      ...
      // With headers, a head pane's state row is its entry's second row and
      // takes Herdr's hang; every other pane's state row is a first row.
      const band = state.bandValue(keys.colorOf.get(entry.workspace), depthIndent, {
        firstRow: headers && !heads.has(entry.pane),
      });
      ...
      const corner =
        grouped && state.INDENT && keys.splitChild(entry)
  ...
  sortKeys(entries, parents, worktrees) {
    ...
    // A pane whose `role` token is `config.leaderRole` leads its workspace: its
    // tab ranks ahead of the workspace's other tabs, and it heads its own split.
    const leaders = config.leaderRole
      ? new Set(entries.filter((entry) => entry.role === config.leaderRole).map((entry) => entry.pane))
      : new Set();
    ...
    for (const [tab, panes] of tabPanes) tabHead.set(tab, panes.find((pane) => leaders.has(pane)) ?? panes[0]);
    const tabKeys = new Map();
    for (const entry of entries) {
      if (!entry.tab) continue;
      const own = minuteKey(entry.pane) ?? NO_STAMP;
      const lead = tabHead.get(entry.tab) && leaders.has(tabHead.get(entry.tab)) ? '1' : '0';
  ...
  paneJobs(
    ...
    // `rowLabel` picks what names the row: the session title, the tab name, or
    // both. A tab-only row with an unnamed tab falls back to the title.
    const tabName = tabs.get(entry.tab) ?? '';
    const tabOnly = config.rowLabel === 'tab' && tabName !== '';
    const title = tabOnly ? tabName : state.trimGroupPrefix(entry.title, header);
    const line = state.composeLine(entry, display, config.rowLabel === 'both' ? tabName : '', indent, spinStep, corner);
  ...
  async groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines) {
    ...
    const fingerprint =
      `${viewMode ?? (grouped ? 'grouped' : 'flat')}:${config.groupHeaders}:${config.groupGap}:` +
    ...
    let wrote;
    if (grouped && config.groupHeaders) {
      wrote = await state.writeGroups(this.src, displayEntries, workspaces, staleWorkspaces, {
        parentOf: keys.parentOf,
        orphanRepo: keys.orphanRepo,
        colorOf,
      });
    } else if (grouped && config.groupGap) {
      // Headers off: a gap still closes each group.
      wrote = await state.writeGaps(this.src, displayEntries, keys.parentOf);
    } else {
      wrote = await state.clearGroups(this.src, entries);
    }
    let ok = wrote.ok;
