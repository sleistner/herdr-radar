...
class Frame {
  ...
  async render(now) {
    ...
    for (const entry of entries) {
      ...
      // indent, so the bar stays at the margin. With headers, a head pane's
      // state row is its entry's second row and takes Herdr's hang; every
      // other pane's state row is a first row.
  ...
  async groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines) {
    ...
    const fingerprint =
      ...
      `${[...keys.colorOf].map(([ws, slot]) => `${ws}=${slot}`).join('+')}:` +
