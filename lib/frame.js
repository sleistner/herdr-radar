...
// Displays a folded group never hides: a question or a finished turn surfaces
// under its header until it is seen.
const ATTENTION = new Set(['blocked', 'done']);
...
class Frame {
  constructor(source) {
    ...
    this.lastFolded = new Map(); // pane -> whether its `folded` token is set
    this.foldViewActive = false; // whether the filter-only view is installed
  ...
  async render(now) {
    ...
    const sortedEntries = this.displayOrder(entries, viewMode, keys);
    const displays = new Map(entries.map((entry) => [entry.pane, this.displayFor(entry, now, deadlines)]));
    // Folding hides rows through a view filter (lib/view.js): the plugin's
    // grouped view carries it, and under the panel's own order it goes on
    // alone while anything is hidden.
    keys.fold = this.foldPlan(sortedEntries, grouped, keys.parentOf, displays, workspaces);
    await this.syncFoldView(viewMode, keys.fold.hidden.size > 0);
    const displayEntries = sortedEntries.filter((entry) => !keys.fold.hidden.has(entry.pane));
    ...
    for (const entry of entries) {
      ...
      this.foldJob(entry.pane, keys.fold.hidden.has(entry.pane), jobs);
      if (entry.workspace) {
  ...
  async groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines) {
    ...
    const colorOf = config.groupColors ? this.groupColorSlots(displayEntries, keys.parentOf) : new Map();
    const fingerprint =
      `${viewMode ?? (grouped ? 'grouped' : 'flat')}:` +
      `${[...keys.fold.hiddenCount].map(([ws, count]) => `${ws}~${count}`).join('+')}:` +
  ...
  // Which panes a folded group hides this frame. Every member workspace of a
  // folded group is hidden except its panes that need attention.
  //
  //   hidden:      panes to hide
  //   hiddenCount: folded top-level workspace -> member workspaces hidden
  //   parents:     top-level workspaces with at least one member on the list
  foldPlan(entries, enabled, parentOf, displays, workspaces) {
    const folded = enabled ? folds.prune(new Set(workspaces.keys())) : new Set();
    const hidden = new Set();
    const hiddenWorkspaces = new Map();
    const parents = new Set();
    for (const entry of entries) {
      const top = parentOf.get(entry.workspace);
      if (!top) continue;
      parents.add(top);
      if (!folded.has(top) || ATTENTION.has(displays.get(entry.pane))) continue;
      hidden.add(entry.pane);
      if (!hiddenWorkspaces.has(top)) hiddenWorkspaces.set(top, new Set());
      hiddenWorkspaces.get(top).add(entry.workspace);
    }
    const hiddenCount = new Map();
    for (const top of folded) {
      if (parents.has(top)) hiddenCount.set(top, hiddenWorkspaces.get(top)?.size ?? 0);
    }
    return { hidden, hiddenCount, parents };
  }

  // Installs the filter-only view while rows are hidden under the panel's own
  // order, and clears it once nothing is. The plugin's own views carry the
  // filter already.
  async syncFoldView(viewMode, anyHidden) {
    if (viewMode !== null) {
      this.foldViewActive = false;
      return;
    }
    if (anyHidden === this.foldViewActive) return;
    const reply = anyHidden ? await view.applyFilterOnly() : await view.clear();
    if (reply && !reply.error) this.foldViewActive = anyHidden;
  }

  // Sets or clears the pane's `folded` token when it changed.
  foldJob(pane, hidden, jobs) {
    if ((this.lastFolded.get(pane) ?? false) === hidden) return;
    jobs.push(
      herdr.reportMetadataAsync(pane, this.src, { folded: hidden ? '1' : null }).then((ok) => {
        if (ok) this.lastFolded.set(pane, hidden);
        return ok;
      }),
    );
  }
