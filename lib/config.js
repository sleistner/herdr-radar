...
module.exports = {
  ...
  // What names an agent row: `title`, `tab` or `both`. Without `row_label`,
  // the older `show_tab = true` reads as `both`.
  rowLabel: ['title', 'tab', 'both'].includes(raw.row_label) ? raw.row_label : raw.show_tab === true ? 'both' : 'title',

  // Paint each top-level group's header, and the headers of every workspace
  // hanging under it, in a colour of its own (palette.groupColors).
  groupColors: raw.group_colors !== false,

  trimGroupPrefix: raw.trim_group_prefix !== false,
  groupGap: raw.group_gap !== false,
  // Header rows naming each workspace. Independent of `groupIndentWidth`.
  groupHeaders: raw.group_headers !== false,
  // The pane `role` token value that puts a pane first in its workspace;
  // empty turns the ordering off.
  leaderRole: typeof raw.leader_role === 'string' ? raw.leader_role.trim() : 'architect',
