...
// title, and the title itself. A tab-only row with an unnamed tab keeps its
// title, so no row goes blank.
//
//   rowText('tab', 'architect', 'Architect::QA') → { tabLabel: '', title: 'architect' }
function rowText(rowLabel, tabName, title) {
  if (rowLabel === 'tab' && tabName) return { tabLabel: '', title: tabName };
