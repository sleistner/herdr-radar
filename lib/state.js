...
// Herdr indents every row of an entry after its first by this many columns.
const CONTINUATION_HANG = 2;

// The stripe for colour `slot`, with `indent` spaces after it so the row's own
// content keeps its depth. A `firstRow` stripe is pushed in by Herdr's hang so
// it lines up with the stripes on continuation rows. Null without a slot.
function bandValue(slot, indent = '', { firstRow = false } = {}) {
  if (slot === undefined) return null;
  const spaces = indent.replace(/\u200b/g, '');
  // Zero-width spaces keep Herdr's trim off the leading and trailing spaces.
  const lead = firstRow ? `\u200b${' '.repeat(CONTINUATION_HANG)}` : '';
  return `${lead}${palette.band.bar}${palette.band.slotMark.repeat(slot + 1)}${spaces ? `${spaces}\u200b` : ''}`;
}
...
// repo: the workspace hangs under its owner whatever it has checked out.
...
async function writeGroups(source, entries, wsLabels, staleWorkspaces = new Set(), tree = {}) {
  ...
  // workspace down belongs to the same family: a parent and its members, and
  // the members among themselves.
  const familyOf = (ws) => parentOf.get(ws) ?? ws;
  const noGap = new Set();
  for (let i = 0; i < order.length - 1; i += 1) {
    if (familyOf(order[i + 1]) === familyOf(order[i])) noGap.add(order[i]);
  }
  const results = await Promise.all(
    entries.map((entry) => {
      ...
      // With no indent there is no tree to draw: a member keeps only its mark.
      const corner = INDENT ? `${lastChild.get(parent) === entry.workspace ? '└' : '├'}─ ` : '';
      const branch = parent ? `${INDENT}${corner}${mark}` : orphan ? `${INDENT ? '└─ ' : ''}${mark}` : '';
      ...
      return herdr.reportMetadataAsync(entry.pane, source, {
        band_head: label === null ? null : bandValue(slot, '', { firstRow: true }),
        band_parent: orphan && name !== null ? bandValue(slot, '', { firstRow: true }) : null,
...
// The furniture without headers: a gap under the last pane of each group,
