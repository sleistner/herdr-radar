...
async function writeGroups(source, entries, wsLabels, staleWorkspaces = new Set(), tree = {}) {
  ...
  const fold = tree.fold ?? { hiddenCount: new Map(), parents: new Set() };
  ...
  const results = await Promise.all(
    entries.map((entry) => {
      ...
      // A group with members carries its fold state: ▾ open, ▸ folded with the
      // count of workspaces it hides.
      const folded = fold.hiddenCount.get(entry.workspace);
      const foldMark = folded !== undefined ? '▸ ' : fold.parents.has(entry.workspace) ? '▾ ' : '';
      const foldCount = folded ? ` (+${folded})` : '';
      const label = name === null ? null : `${branch}${foldMark}${name}${foldCount}`;
