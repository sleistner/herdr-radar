...
// Hides the rows a folded group covers (lib/frame.js foldPlan).
const FOLD_FILTER = { op: 'not', filter: { op: 'exists', field: { token: 'folded' } } };
...
function apply(mode) {
  ...
  const params = { source: source(), label: spec.label, sort: spec.sort };
  if (spec.filter) params.filter = spec.filter;
  return call('agent.view.set', params);
}

// The fold filter on its own. With no `sort`, the panel keeps its own order.
function applyFilterOnly() {
  return call('agent.view.set', { source: source(), label: 'folded', filter: FOLD_FILTER });
}
...
module.exports = { call, apply, applyFilterOnly, clear, mode, setMode, setRows, resolve, DEFAULT_MODE };
